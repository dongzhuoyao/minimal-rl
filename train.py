...
1. **SAMPLING PHASE** (Lines 346-445):
   - For each batch of labels, sample multiple trajectories (num_samples_per_prompt)
...
================================================================================
    ...
    python train.py training.batch_size=8 training.num_samples_per_prompt=4
...
class PerLabelStatTracker:
    ...
    def update(self, labels, rewards, type='grpo'):
        ...
        # Check for NaN/Inf in input rewards
        if np.any(~np.isfinite(rewards)):
            print(f"Warning: NaN/Inf detected in input rewards to stat_tracker.update()")
            rewards = np.where(np.isfinite(rewards), rewards, np.zeros_like(rewards))
        
        unique = np.unique(labels)
        advantages = np.zeros_like(rewards)  # Initialize with zeros
        ...
        for label in unique:
            ...
            # Check for NaN/Inf in mean or std
            if np.any(~np.isfinite(mean)) or np.any(~np.isfinite(std)) or np.any(std <= 0):
                print(f"Warning: Invalid stats for label {label}: mean={mean}, std={std}")
                std = np.maximum(std, 1e-4)  # Ensure std is positive
                mean = np.where(np.isfinite(mean), mean, np.zeros_like(mean))
            
            if type == 'grpo':
                advantages[labels == label] = (label_rewards - mean) / std
                
                # Check for NaN/Inf in advantages
                if np.any(~np.isfinite(advantages[labels == label])):
                    print(f"Warning: NaN/Inf detected in advantages for label {label}")
                    advantages[labels == label] = np.where(
                        np.isfinite(advantages[labels == label]),
                        advantages[labels == label],
                        np.zeros_like(advantages[labels == label])
                    )
            elif type == 'rwr':
...
def compute_log_prob_at_timestep(model, x_t, t, labels, x_next, dt):
    ...
    # Check for NaN/Inf in velocity prediction
    if torch.any(~torch.isfinite(v_pred)):
        print(f"Warning: NaN/Inf detected in v_pred")
        v_pred = torch.where(
...
def main(cfg: DictConfig):
    ...
    while step < cfg.training.max_steps:
        ...
        # Check for NaN/Inf in images before reward computation
        if torch.any(~torch.isfinite(images_for_reward)):
            print(f"Warning: NaN/Inf detected in images_for_reward at step {step}")
            images_for_reward = torch.where(
                torch.isfinite(images_for_reward),
                images_for_reward,
                torch.zeros_like(images_for_reward)
            )
        ...
        if torch.any(~torch.isfinite(rewards)):
            print(f"Warning: NaN/Inf detected in rewards at step {step}")
            rewards = torch.where(
        ...
        # Check again after conversion to numpy
        if np.any(~np.isfinite(rewards)):
            print(f"Warning: NaN/Inf detected in rewards (numpy) at step {step}")
            rewards = np.where(np.isfinite(rewards), rewards, np.zeros_like(rewards))
        ...
        for inner_epoch in range(num_inner_epochs):
            ...
            for batch_start in range(0, len(samples), train_batch_size):
                ...
                for j in range(num_train_timesteps):
                    ...
                    # Check for NaN/Inf in log_prob_new
                    if torch.any(~torch.isfinite(log_prob_new)):
                        print(f"Warning: NaN/Inf detected in log_prob_new at step {step}, timestep {j}")
                        log_prob_new = torch.where(
                            torch.isfinite(log_prob_new),
                            log_prob_new,
                            torch.zeros_like(log_prob_new)
                        )
                    
                    # Get old log probability for this timestep
                    log_prob_old = batch_log_probs_old[:, j]  # [B]
                    
                    # Check for NaN/Inf in log_prob_old
                    if torch.any(~torch.isfinite(log_prob_old)):
                        print(f"Warning: NaN/Inf detected in log_prob_old at step {step}, timestep {j}")
                        log_prob_old = torch.where(
                            torch.isfinite(log_prob_old),
                            log_prob_old,
                            torch.zeros_like(log_prob_old)
                        )
                    ...
                    # Clip log_prob difference to prevent exponential overflow
                    log_prob_diff = log_prob_new - log_prob_old  # [B]
                    max_log_diff = cfg.training.get("max_log_diff", 10.0)  # Prevent exp overflow
                    log_prob_diff_clipped = torch.clamp(log_prob_diff, -max_log_diff, max_log_diff)
                    ratio = torch.exp(log_prob_diff_clipped)  # [B]
                    
                    # Check for NaN/Inf in ratio and replace with 1.0 (no update)
                    ratio = torch.where(
                    ...
                    # Check for NaN/Inf in policy_loss
                    if not torch.isfinite(policy_loss):
                        print(f"Warning: NaN/Inf detected in policy_loss at step {step}, timestep {j}")
                        print(f"  ratio stats: min={ratio.min().item():.4f}, max={ratio.max().item():.4f}, mean={ratio.mean().item():.4f}")
                        print(f"  advantages stats: min={advantages_clipped.min().item():.4f}, max={advantages_clipped.max().item():.4f}, mean={advantages_clipped.mean().item():.4f}")
                        print(f"  log_prob_new stats: min={log_prob_new.min().item():.4f}, max={log_prob_new.max().item():.4f}, mean={log_prob_new.mean().item():.4f}")
                        print(f"  log_prob_old stats: min={log_prob_old.min().item():.4f}, max={log_prob_old.max().item():.4f}, mean={log_prob_old.mean().item():.4f}")
                    ...
                    if beta > 0:
                        ...
                        if not torch.isfinite(kl_loss):
                            print(f"Warning: NaN/Inf detected in kl_loss at step {step}, timestep {j}")
                    ...
                    if not torch.isfinite(loss):
                        print(f"Warning: NaN/Inf detected in loss at step {step}, timestep {j}, skipping this timestep")
